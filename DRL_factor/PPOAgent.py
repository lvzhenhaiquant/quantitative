...
class PPOAgent:
    ...
    def learn_old(self):
        ...
        for _ in range(self.num_epochs):
            batchs_data= self.buffer.generate_batches()
            for batch in batchs_data:
                ...
                batch_states = [batch_stock_states, batch_token_states,batch_ast_states]
                # 使用优化的训练步骤
                actor_loss, critic_loss, entropy = self.distributed_train_step(
                    batch_states, batch_actions, batch_old_probs, batch_returns, batch_advantages)
